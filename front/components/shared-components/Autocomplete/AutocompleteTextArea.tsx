...
export default function AutocompleteTextArea({ value, onChange, placeholder }: any) {
    ...
    const onSelect = (selectedValue: string) => {
        if (position >= 2) {
            const newValue = value.slice(0, position - 3) + `<<${selectedValue}>>` + value.slice(position);
            onChange(newValue);
            setOpen(false);
        }
    };
        ...
        <div className='flex flex-col w-full h-full'>
            ...
            <Popover open={open} onOpenChange={setOpen}>
                ...
                <PopoverContent className="w-64 p-0">
                    <Command>
                        <CommandInput placeholder='Variable' />
                        <CommandList>
